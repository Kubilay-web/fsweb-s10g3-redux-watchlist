...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ADD_FAVORITE: {
      ...
      if (!state.favourites.some((movie) => movie.id === favoriteMovie.id)) {
        return {
          ...state,
          favourites: [...state.favourites, favoriteMovie],
          movies: state.movies.filter((movie) => movie.id !== favoriteMovie.id),
        };
      }
      return state;
    }
    case REMOVE_FAVORITE: {
      ...
      const newFavMovies = state.favourites.filter(
        (favMovie) => favMovie.id !== removedMovieId
      );
